...
Input and expected output files are stored in the [`DATA/`](TASKS/TASK05-VSAM-BANKING/DATA) folder:
...
2. **Load initial master data** — load `ACCT.MASTER.BEFORE` into the KSDS cluster either via REPRO (see [`DATAVSAM.jcl`](../JCLSAMPLES/DATAVSAM.jcl)) or manually through **File Manager** in ISPF (option 3.4 → open VSAM dataset → edit records directly)
